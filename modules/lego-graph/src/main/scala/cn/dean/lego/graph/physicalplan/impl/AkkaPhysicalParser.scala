...
package cn.dean.lego.graph.physicalplan.impl
...
import cn.dean.lego.graph.physicalplan.{PhysicalParser, NodeIn, NodeOut, NotifyActor}
...
class AkkaPhysicalParser(implicit injector: Injector) extends PhysicalParser[NodeProp, RunnableGraph[Future[Seq[ComponentResult]]]] with AkkaInjectable {
  ...
  logger.info(s"Start up notification [${notifyActor.path}]")
