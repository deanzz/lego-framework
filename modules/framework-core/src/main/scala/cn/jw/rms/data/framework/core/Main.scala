...
import cn.jw.rms.data.framework.common.log.Logger
import cn.jw.rms.data.framework.common.rules.AssemblyResult
import cn.jw.rms.data.framework.common.utils.{MailAPI, TimerMeter, WechatAPI}
import cn.jw.rms.data.framework.core.config._
...
object Main {
  ...
  //val logger: Logger = null //= LoggerFactory.getLogger(this.getClass)
  val TYPE_SYSTEM = "system"
  val TYPE_APPLICATION = "application"
  val TYPE_MODULE = "module"
...
//spark-submit --jars $(echo ${ROOT_DIR}/assemblies/*.jar | tr ' ' ',') --class cn.jw.rms.data.framework.core.Entrance --master yarn-client --executor-memory 5G --driver-memory 8G --num-executors 10 --conf spark.default.parallelism=30 --conf spark.shuffle.consolidateFiles=true --conf spark.rdd.compress=true ${ROOT_DIR}/framework-core-assembly-1.3.jar ${ROOT_DIR}/conf/application.conf
