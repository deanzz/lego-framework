...
spark-submit --conf spark.app.name=$MODEL_NAME --jars $(find $ROOT_DIR -type f -name "*.jar" -not -name "lego-core*" | awk -F/ '{ $0=$0 ; print $NF","$0}' | sort -u -t, -k1,1 | cut -d"," -f2 | xargs echo | tr ' ' ',') --class cn.jw.rms.data.framework.core.Main --master yarn --deploy-mode $DEPLOY_MODE $SPARK_PARAMETERS "$SUBMIT_DIR/$LEGO_CORE_JAR" $SUBMIT_DIR/conf/application.conf
